...
## Introduction
I am going to use a sample dataset, called mtcars. Using this, I will list variable names, generate *summary statistics*, create plot, do regressions, some statistics tests.  
Variable names are:  
**`r colnames(mtcars)`**  
[link to codebook](https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/mtcars)

### Descriptions
Basic summary stats based on variables. I am interested in the miles per gallon, weight of car, Automatic or manual(VS) and horsepower(hp).
...
plot(mtcars$mpg, mtcars$wt, main = "Scatter plot showing relationship between Miles per Gallon and Weight of car", xlab = "Miles per Gallon(MPG)", ylab = "Weight in pounds(wt)")
```
